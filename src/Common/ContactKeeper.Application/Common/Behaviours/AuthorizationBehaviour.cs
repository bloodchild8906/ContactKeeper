...
namespace ContactKeeper.Application.Common.Behaviours;
...
public class AuthorizationBehaviour<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse>
{
    ...
    public async Task<TResponse> Handle(TRequest request, CancellationToken cancellationToken, RequestHandlerDelegate<TResponse> next)
    {
        ...
        if (authorizeAttributes.Any())
        {
            ...
            if (_currentUserService.UserId.Equals(null))
            {
            ...
            if (authorizeAttributesWithRoles.Any())
            {
                foreach (var roles in authorizeAttributesWithRoles.Select(a => a.Roles.Split(',')))
                {
                    ...
                    if (!authorized)
                    {
                        _logger.LogInformation("Matech.ContactKeeper Authorization Request: {@UserId} {@Request}", _currentUserService.UserId, request);
